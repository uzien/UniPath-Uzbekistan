...
export default function ProfileDrawer({ profile, onUpdateProfile, isOpen, onClose, t }: ProfileDrawerProps) {
      ...
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-[#F2F2F7] p-6 pb-0">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-bold text-[#1C1C1E] tracking-tight">
            {t.academicSettings || 'My Academic Profile'}
          </h2>
        </div>
        
        <div className="flex items-center gap-3">
          {/* Quick templates */}
          <span className="text-[11px] font-bold text-[#8E8E93] uppercase tracking-widest hidden lg:inline">
          ...
          <div className="flex flex-wrap gap-1.5 sm:gap-2">
            <button
              ...
              id="template-avg-btn"
              className="px-2.5 py-1 text-[10px] sm:text-[11px] font-bold bg-[#F2F2F7] hover:bg-[#E5E5EA] rounded-lg text-[#3A3A3C] border border-[#E5E5EA] ios-transition active:scale-95 cursor-pointer"
            >
              {t.avgStudent || 'Avg School Student'}
            </button>
            <button
              onClick={() => applyTemplate('high')}
              id="template-high-btn"
              className="px-2.5 py-1 text-[10px] sm:text-[11px] font-bold bg-blue-50/50 hover:bg-blue-50 text-[#007AFF] border border-blue-100/50 rounded-lg ios-transition active:scale-95 cursor-pointer"
            >
              {t.lyceumAchiever || 'Lyceum Achiever'}
            </button>
            <button
              onClick={() => applyTemplate('presidents')}
              id="template-president-btn"
              className="px-2.5 py-1 text-[10px] sm:text-[11px] font-bold bg-indigo-50/50 hover:bg-indigo-50 text-indigo-600 border border-indigo-100/50 rounded-lg ios-transition active:scale-95 cursor-pointer"
            >
              {t.presidentGrad || 'Ivy / President Grad'}
            </button>
          </div>
          ...
          <button
            ...
            title={t.closeSettings || "Minimize control board"}
          >
            <X className="w-5 h-5" />
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 text-[#3A3A3C] p-6 pt-0">
        ...
        <div className="space-y-3">
          <label className="block text-xs font-bold uppercase tracking-widest text-[#8E8E93]">
            {t.uzbekDiplomaStatus || 'Uzbek Diploma Type'}
          </label>
          <div className="space-y-2">
            {[
              { id: 'school', label: t.diplomaTypeSchool || '11-Year School (Middle)' },
              { id: 'lyceum', label: t.diplomaTypeLyceum || '12-Year Academic Lyceum' },
              { id: 'college', label: t.diplomaTypeCollege || '12-Year Vocational College' },
              { id: 'ib_ap', label: t.diplomaTypeIB || 'International (IB/AP)' }
            ].map((option) => (
              <button
                key={option.id}
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            <label className="text-xs font-bold uppercase tracking-widest text-[#8E8E93]">
              {t.ieltsTarget ? 'IELTS Academic' : 'IELTS Academic'}
            </label>
          ...
          <div className="bg-[#F2F2F7] border border-[#E5E5EA] p-3 rounded-xl text-[10px] font-semibold text-[#8E8E93] leading-relaxed">
            {t.ieltsExplain || "IELTS measures listening, reading, writing, and speaking. Most standard degrees request 6.0 - 6.5."}
          </div>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            <label className="text-xs font-bold uppercase tracking-widest text-[#8E8E93]">
              {t.satLabel || 'SAT Score'}
            </label>
            <span className="text-sm font-bold text-[#007AFF] bg-blue-50 px-2.5 py-1 rounded-lg">
              {profile.sat !== null ? `${profile.sat} ${t.pts || 'pts'}` : t.noSat || 'No SAT score'}
            </span>
          </div>
          ...
          <div className="space-y-3">
            <label className="flex items-center gap-2 cursor-pointer py-1">
              ...
              <span className="text-xs font-semibold text-[#3A3A3C]">
                {t.satTaken || 'I have taken the SAT test'}
              </span>
            </label>
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            ...
            <span className="text-xs font-bold text-[#007AFF] bg-blue-50 px-2 py-0.5 rounded">
              {t.usScale || 'US Scale equivalent:'} {profile.gpa} / 4.0
            </span>
          </div>
            ...
            <div className="pt-2">
              ...
              <div className="flex justify-between text-[10px] text-[#8E8E93] font-semibold mt-2">
                ...
                <span className="text-[#1C1C1E] font-bold">
                  {t.currentLabel || 'Current:'} {profile.originalGpa} / {profile.gpaScale === 100 ? '100' : profile.gpaScale}
                </span>
